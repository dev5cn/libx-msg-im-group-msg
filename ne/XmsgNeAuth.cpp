...
void XmsgNeAuth::handle(shared_ptr<XscChannel> channel, SptrXitp trans, shared_ptr<XmsgNeAuthReq> req)
{
	...
	rsp->set_cgt(XmsgImGroupCfg::instance()->cfgPb->cgt());
	trans->end(rsp);
	if (old == nullptr)
		return;
...
shared_ptr<XmsgImGroupCfgXmsgNeN2hAddr> XmsgNeAuth::findXmsgNeCfg(const string& cgt)
{
	for (int i = 0; i < XmsgImGroupCfg::instance()->cfgPb->n2h_size(); ++i)
	{
		auto ne = XmsgImGroupCfg::instance()->cfgPb->n2h(i);
		if (ne.cgt() != cgt)
			continue;
		shared_ptr<XmsgImGroupCfgXmsgNeN2hAddr> pb(new XmsgImGroupCfgXmsgNeN2hAddr());
		pb->CopyFrom(ne);
		return pb;
	}
	return nullptr;
